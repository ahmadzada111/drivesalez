...
public class RenewLimitsForDefaultUserJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        //         .Any(r => r.Id == joined.UserRole.RoleId && (r.Name == UserType.DefaultUser.ToString())))
        //     .Select(joined => joined.User)
        //     .ToListAsync();
        //
        // var limit = await _dbContext.UserLimits
        //     .Where(x => x.UserType == UserType.DefaultUser)
        //     .FirstOrDefaultAsync() ??
        //     throw new InvalidOperationException($"Limit with {UserType.DefaultUser} - type wasn't found");
        //
